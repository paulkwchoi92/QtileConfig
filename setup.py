...
def print_program_welcome():
    ...
    welcome = "SETUP SCRIPT OF PAUL'S QTILE CONFIG".center(
        len(row) - len(sep) * 2)
    ...
    print(
        f"Hi there {getuser()}, you are about to setup Qtile".center(len(row)))
...
def get_dependencies(software_path="software.txt"):
    ...
    if not os.path.exists(software_path):
        print(
            f"The file {software_path} wasn't found. Please clone again or provide one")
